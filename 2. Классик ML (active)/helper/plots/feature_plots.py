...
class ClassificationPlot():
    ...
    def metrics_plot(self):
        ...
        sns.barplot({k: v for k, v in self.metrics[-5]['0'].items() if k != 'support'}, ax=ax[0][0]);
        sns.barplot({k: v for k, v in self.metrics[-5]['1'].items() if k != 'support'}, ax=ax[0][0], color='lightpink');
        ax[0][0].set_title('Classification Report')

        ax[0][1].plot(self.metrics[-1][0], self.metrics[-1][1])
        ax[0][1].set_title('PR Curve')

        sns.heatmap(self.metrics[-4], annot=True, fmt='d', cmap='Blues', xticklabels=['Normal', 'Fraud'], yticklabels=['Normal', 'Fraud'], ax=ax[1][0]);
        ax[1][0].set_title('Confusion Matrix')

        ax[1][1].plot(self.metrics[-3][0], self.metrics[-3][1])
        ax[1][1].plot([0, 1], [0, 1], 'k--')
        ax[1][1].set_title('Roc-Auc Curve')
